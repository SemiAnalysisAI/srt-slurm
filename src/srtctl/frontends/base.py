...
FrontendType = Literal["dynamo", "sglang", "sglang-router", "trtllm_serve", "vllm", "vllm-router", "tilert-router"]
...
def get_frontend(frontend_type: str) -> FrontendProtocol:
    ...
    if frontend_type == "dynamo":
        ...
        return VLLMFrontend()
    elif frontend_type == "tilert-router":
        return TileRTRouterFrontend()
    elif frontend_type == "vllm-router":
        return VLLMRouterFrontend()
    else:
        raise ValueError(
            f"Unknown frontend type: {frontend_type!r}. Supported: dynamo, sglang, sglang-router, trtllm_serve, vllm, vllm-router, tilert-router"
        )
