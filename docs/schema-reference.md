...
| `engine` | str \| mapping | required | The engine type (`atom`, `sglang`, `tilert`, `trtllm`, `vllm`, `mocker`) as a string, or a mapping with `type` plus the engine-wide knobs listed under [Engine types](#engine-types). |
...
| `prefill_container` | str \| None | `None` | Image or cluster container alias for prefill workers. |
| `model_profile` | str | `'glm5'` | TileRT P/D model profile (decode_server --model, connector tilert_model). |
| `weights_dir` | str \| None | `None` | Converted TileRT weights, at this container path in every decode worker. |
...
| `prefill_kv_cache_dtype` | str | `'fp8_ds_mla'` | vLLM prefill KV cache dtype; must match the layout decode expects. |
| `transport` | one of `'nixl'`, `'mooncake'` | `'nixl'` | KV transfer data plane shared by prefill and decode. |
| `with_mtp` | bool | `True` | Enable MTP on both sides of the P/D transfer. |
| `speculative_tokens` | int | `1` | Speculative tokens advertised by vLLM prefill when with_mtp is set. |
| `weight_converter` | [TileRTWeightConverter](#tilertweightconverter) \| None | `None` | Optional conversion into weights_dir when it has no complete conversion. |
...
| Key | Type | Default | Description |
|---|---|---|---|
| `module` | str | required | Python module of the converter, run with ``python -m`` (for example ``tilert.models.preprocess.weight_converter``). |
| `args` | dict[str, Any] | `{}` | Converter arguments; keys keep their spelling (``model_type``, ``num_mtp``). srtctl supplies ``--model_dir`` and ``--save_dir``. |
| `ready_file` | str | `'model.safetensors.index.json'` | Path relative to ``weights_dir`` that exists only in a complete conversion. Its presence skips conversion. |
