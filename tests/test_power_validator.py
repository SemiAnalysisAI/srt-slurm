...
def test_default_nvidia_bundle_matches_pre_profile_bytes(package, monkeypatch):
    # Generated by this suite's existing package fixture at PR #14's head,
    # 03863ab23804f8a31811d9aaccc54abc8961a41c. Pin only the build version:
    # hatch-vcs changes it with the checkout, independently of wire behavior.
    # Refactor tripwire and regeneration guide: fixtures/nvidia-power-golden.md.
    monkeypatch.setattr("srtctl.core.power.manifest.PRODUCER_VERSION", "golden-test")
    log_dir, power_dir = package()
    golden = Path(__file__).parent / "fixtures" / "nvidia-power-golden"
    expected = {path.relative_to(golden): path.read_bytes() for path in golden.rglob("*") if path.is_file()}
...
@pytest.mark.parametrize(
    "defect",
    ["null", "incomplete", "invalid_label", "non_string", "malformed_sources", "metric", "scope", "utilization"],
)
def test_offline_validator_rejects_inconsistent_custom_profile_provenance(package, defect):
    ...
    if defect == "null":
        ...
        manifest["power_profile"]["power_metric"] = 12
    elif defect == "malformed_sources":
        manifest["power_profile"]["utilization_sources"] = ["gpu_util_pct", "amd_smi_gfx_activity_percent"]
    elif defect == "metric":
...
def test_validator_accepts_unknown_profile_provenance(package):
